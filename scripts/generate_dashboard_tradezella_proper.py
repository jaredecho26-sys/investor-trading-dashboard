#!/usr/bin/env python3
"""
Proper TradeZella-inspired dashboard.
- Soft background (#F5F6F8)
- Purple/indigo brand color (#6366F1)
- Pastel color palette
- Card-based layout with generous whitespace
- Micro-visualizations in metric cards
- Calendar heatmap for daily P&L
...
def generate_calendar_heatmap(history):
    """Generate calendar heatmap data for the last 60 days"""
    if len(history) < 2:
        return []
    ...
    for i in range(1, min(61, len(history))):
        ...
        if daily_return > 2:
            ...
            intensity = 1  # lightest
        elif daily_return > -2:
            intensity = -2
        else:
            intensity = -4  # darkest red
            ...
            <div class="card">
                ...
                <div class="sidebar">
                    <div class="stat-row">
                        ...
                        <span class="stat-value" style="color: {COLORS['green']}">{win_rate:.1f}%</span>
                    </div>
                    <div class="stat-row">
                    ...
                    <div class="stat-row">
                        ...
                        <span class="stat-value" style="color: {COLORS['green']}">{fmt_pct(best_day)}</span>
                    </div>
                    <div class="stat-row">
                        <span class="stat-label">Worst Day</span>
                        <span class="stat-value" style="color: {COLORS['red']}">{fmt_pct(worst_day)}</span>
                    </div>
                    <div class="stat-row">
                        <span class="stat-label">Drawdown</span>
                        <span class="stat-value" style="color: {COLORS['red']}">{fmt_pct(drawdown)}</span>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="card">
            ...
            <div class="performance-grid">
                ...
                <div class="perf-card">
                    ...
                    <div class="perf-value" style="color: {COLORS['green']}">{win_rate:.0f}%</div>
                </div>
                <div class="perf-card">
                    <div class="perf-label">Best Day</div>
                    <div class="perf-value" style="color: {COLORS['green']}">{fmt_pct(best_day)}</div>
                </div>
                <div class="perf-card">
                    <div class="perf-label">Worst Day</div>
                    <div class="perf-value" style="color: {COLORS['red']}">{fmt_pct(worst_day)}</div>
                </div>
            </div>
        </div>
    </div>
...
"""
    ...
    print(f"✓ TradeZella-proper dashboard generated: {DASHBOARD_FILE}")
    return True
